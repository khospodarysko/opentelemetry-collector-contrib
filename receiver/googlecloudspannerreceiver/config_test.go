...
const (
	cardinalityOverallLimit = 200000
)
...
func TestValidateConfig(t *testing.T) {
	...
	testCases := map[string]struct {
		...
		"All required fields are populated":                   {defaultCollectionInterval, defaultTopMetricsQueryMaxRows, cardinalityOverallLimit, []Project{project}, false},
		"Invalid collection interval":                         {-1, defaultTopMetricsQueryMaxRows, cardinalityOverallLimit, []Project{project}, true},
		"Invalid top metrics query max rows":                  {defaultCollectionInterval, -1, cardinalityOverallLimit, []Project{project}, true},
		"Top metrics query max rows greater than max allowed": {defaultCollectionInterval, defaultTopMetricsQueryMaxRows + 1, cardinalityOverallLimit, []Project{project}, true},
		"No projects":                           {defaultCollectionInterval, defaultTopMetricsQueryMaxRows, cardinalityOverallLimit, nil, true},
		"Invalid project in projects":           {defaultCollectionInterval, defaultTopMetricsQueryMaxRows, cardinalityOverallLimit, []Project{{}}, true},
		"Cardinality overall limit is zero":     {defaultCollectionInterval, defaultTopMetricsQueryMaxRows, 0, []Project{project}, false},
		"Cardinality overall limit is negative": {defaultCollectionInterval, defaultTopMetricsQueryMaxRows, -cardinalityOverallLimit, []Project{project}, true},
	}
