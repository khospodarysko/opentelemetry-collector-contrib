...
func (generator timestampsGenerator) pullTimestamps(lastPullTimestamp time.Time, now time.Time) []time.Time {
	var timestamps []time.Time
	upperBound := shiftToStartOfMinute(now)

	if lastPullTimestamp.IsZero() {
		if generator.backfillEnabled {
		...
		timestamps = pullTimestampsWithDifference(lastPullTimestamp, upperBound, generator.difference)
	}
