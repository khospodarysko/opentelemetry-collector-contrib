...
func (tcr mockCompositeReader) Read(ctx context.Context) []pdata.Metrics {
	md := pdata.NewMetrics()
...
func TestStartWithCollectDataError(t *testing.T) {
	logger := zap.NewNop()
	...
	receiverCasted.projectReaders = []statsreader.CompositeReader{mockCompositeReader{}}

	time.Sleep(cfg.CollectionInterval + time.Second)
